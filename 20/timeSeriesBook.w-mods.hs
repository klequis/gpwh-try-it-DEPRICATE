...
{-
    Combine the files into one timeseries
-}
...
{-
    Calculate the min a max values
-}
...
-- -- compareTS func (TS [] []) = Nothing
-- inCommon :: (Real a) => TS a -> TS a -> TS a
-- inCommon (TS [] []) _ = TS [] []
-- inCommon _ (TS [] []) = TS [] []
-- inCommon (TS t1 v1) (TS t2 v2) = v1inv2
--   where v1inv2 = foldl (\x -> x `elem` v2) v1
